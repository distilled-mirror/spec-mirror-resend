...
<Note>
  ...
  Broadcasts](/docs/dashboard/broadcasts/introduction), instead.
</Note>
...
If the request fails, the response will include an `error` object with a `message` property. You can find more information about the error in the [Errors](/docs/api-reference/errors) section of the API Reference.
...
All emails sent via the batch endpoint appear in the [Emails page](https://resend.com/emails) of your dashboard along with individually sent emails. Each email will have a `queued` status initially before being processed.

## API Reference

For complete API documentation, see the [Send Batch Emails API reference](/docs/api-reference/emails/send-batch-emails).
