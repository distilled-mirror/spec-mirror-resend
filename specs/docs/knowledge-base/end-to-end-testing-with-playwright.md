...
<Steps>
  ...
  <Step title="Write the test spec file">
    Create a test spec file at `e2e/app.spec.ts`. You can test in two ways: by calling the Resend API or by mocking a response.
    ...
    <Note>
      ...
      [test accounts](/docs/dashboard/emails/send-test-emails) run through the entire API
      flow without harming your reputation.
    </Note>
  </Step>

  <Step title="Create a Playwright config file">
